...
export async function execute(sock, msg, args) {
  ...
  try {
    await sock.sendMessage(from, {
      text: `⚽ *Dribble d'expulsion en cours...*\n➡️ Expulsion de @${cible.split("@")[0]}`,
      mentions: [cible]
    }, { quoted: msg });
  } catch {}
  ...
  try {
    ...
    await sock.sendMessage(from, {
      text: `👟 L'utilisateur @${cible.split("@")[0]} a été dribblé hors du groupe ! ⚽`,
      mentions: [cible]
    });
  } catch (e) {
    await sock.sendMessage(from, {
      text: "❌ Impossible d'expulser cet utilisateur (permissions insuffisantes ?)",
    }, { quoted: msg });
  }
}